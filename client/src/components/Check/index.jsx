...
// 合併所有的 Ant Design 組件
import { Button, Spin, Input, Flex, Card, Row, Col, Statistic, App } from "antd";
...
const Check = ({ feedback, isChecking, onTutorClick, stage, question }) => {
  ...
  const stageResult = feedback != null ? feedback : stage ? byStage?.[stage] || null : null;
  ...
  const preparePayload = async (stageNum) => {
    ...
    if (stageNum === 1) {
      ...
      const dataUrl = await toPng(reactFlowWrapper, { backgroundColor: "#ffffff", pixelRatio: 2 });
      return { ...basePayload, graph: { nodes, edges }, imageBase64: dataUrl.split(",")[1] };
    } else if (stageNum === 2) {
  ...
  const handleCheck = async () => {
    ...
    setIsTyping(true);
    try {
      setMessages((prev) => [...prev, { sender: "assistant", text: `好的！讓我檢查中... 🔍` }]);
      ...
      setMessages((prev) => [
        ...prev.slice(0, -1),
        { sender: "assistant", text: `## 檢查結果\n\n${data.checkReport || "已完成檢查"}\n\n有任何問題都可以問我！` },
      ]);
    } catch (error) {
      setMessages((prev) => [...prev.slice(0, -1), { sender: "assistant", text: `❌ ${error.message}` }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const renderMessage = (msg, index) => (
    <div key={index} className={msg.sender === "assistant" ? styles.rowLeft : styles.rowRight}>
      {msg.sender === "assistant" && <div className={styles.avatar}>🤖</div>}
      <div className={msg.sender === "assistant" ? styles.bubbleLeft : styles.bubbleRight}>
        <ReactMarkdown>{msg.text}</ReactMarkdown>
      </div>
      {msg.sender === "user" && <div className={styles.avatarUser}><UserOutlined /></div>}
    </div>
  );
        ...
        <div className={styles.container}>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {onTutorClick && (
              <div className={styles.tutorbox}>
                <Button style={{ height: "36px", fontSize: "16px", backgroundColor: "#375bd3", color: "#FFFFFF" }} onClick={onTutorClick}>
                  詢問沐芙助教
                </Button>
              </div>
            )}
          ...
          <div style={{ height: "80%" }}>
            <div className={styles.topicbox}>
              {isChecking && <div style={{ textAlign: "center", padding: "10px" }}><Spin size="small" tip="AI 辨識中..." /></div>}

              {scores && checkFeedback && (
                <div style={{ marginBottom: "20px", borderBottom: "1px solid #eee", paddingBottom: "20px" }}>
                  <h3 style={{ color: "#9287EE", marginBottom: "16px" }}>檢查結果報告</h3>
