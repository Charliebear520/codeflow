...
const Answer = () => {
  ...
  /*const handleSave = async () => {
    try {
      ...
      if (!flowRef.current?.exportGraph) { message.error("流程圖元件尚未載入"); return; }
    ...
    if (flowElement) {
        imageBase64 = await toPng(flowElement, { backgroundColor: "#fff", pixelRatio: 2 });

      const dataUrl = await toPng(flowElement, {
        backgroundColor: "#ffffff",
        pixelRatio: 2,
      });
      imageBase64 = dataUrl; // 包含 data:image/png;base64, 前綴
    }

      const token = await getToken();
      const payload = {
        questionId: "Q001",        // 實際題號來源
        graph: { nodes, edges },   //  ★ 關鍵：把 React Flow 的 JSON 送出
        imageBase64,               //  把流程圖的 Base64 圖片也送出去
        mode: "editor",            // 你也有「上傳圖片」模式，這裡標示來源
        completed: false,          // 這階段先不標示完成
      };
    ...
    // const payload = {
    //   questionId: "Q001",        // 你實際的題號來源
    //   userId: "student-123",     // 可選：登入的使用者ID
    //   mode: "editor",            // 你也有「上傳圖片」模式，這裡標示來源
    //   graph: { nodes, edges },   // ★ 關鍵：把 React Flow 的 JSON 送出去
    // };
    ...
    //   message.success("已儲存作答");
    // } else {
    //   message.error(`儲存失敗：${data.error || res.statusText}`);
    // }
  }*/
